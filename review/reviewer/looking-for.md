...
reviewer to check a PR's behavior is when it has a user-facing impact, such as
a **UI change**. It's hard to understand how some changes will impact a user
when you're just reading the code. For changes like that, you can have the
developer include screenshots, screen recordings, or give you a demo of the
functionality if it's too inconvenient to patch in the PR and try it yourself.
...
Projects should have a style guide at Airship.  Make sure the PR follows the
...
see that it also updates associated documentation, including READMEs, quip
pages, and any generated reference docs. If the PR deletes or deprecates code,
consider whether the documentation should also be deleted.  If documentation is
missing, ask for it.
...
and wait for them to clarify it before you try to review it. At Airship, we hire
...
-   Code is appropriately documented (generally in the codebase or quip).
